...
function th_masterslider_update_check() {
    ...
    if ( is_dir( $th_plugin_dir ) ) {
        ...
        $myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
            'https://github.com/ryanlabelle/'.$th_plugin_slug.'/',
...
function themo_ot_meta_box_post_format_quote($array,$pages) {
	...
	$array['fields'] = array(
		...
		array(
			...
			'label'   => '',
			'desc'    => esc_html__( 'Source Name (ex. author, singer, actor)', 'bellevue' ),
			'std'     => '',
			'type'    => 'text'
		),
		array(
			...
			'label'   => '',
			'desc'    => esc_html__( 'Source Title (ex. book, song, movie)', 'bellevue' ),
			'std'     => '',
			'type'    => 'text'
		));
	return $array;
...
function themo_ot_meta_box_post_format_audio($array,$pages) {
	...
	$array['fields'] = array(
		array(
			...
			'desc'    => esc_html__( 'Use the built-in <code>[audio]</code> shortcode here.', 'bellevue' ),
			'std'     => '',
			'type'    => 'textarea'
		)
	);
...
function themo_ot_meta_box_post_format_link($array,$pages) {
	...
	$array['fields'] = array(
		...
		array(
			'id'      => '_format_link_url',
			'label'   => '',
			'desc'    => esc_html__( 'Link URL (ex. http://google.com)', 'bellevue' ),
			'std'     => '',
			'type'    => 'text'
		),
		array(
			...
			'label'   => '',
			'desc'    => esc_html__( 'Link Title (ex. Check out Google)', 'bellevue' ),
			'std'     => '',
			'type'    => 'text'
		),
		...
		array(
			...
			'label'       => esc_html__( 'Link Target', 'bellevue' ),
			'type'        => 'checkbox',
			'choices'     => array(
				array(
					'value'       => '_blank',
		...
		/*array(
			...
			'label'       => esc_html__( 'Link behaviour on the portfolio homepage', 'bellevue' ),
...
function themo_ot_meta_box_post_format_video($array,$pages) {
	...
	$array['fields'] = array(
		array(
			...
			'desc'    => sprintf( wp_kses_post( __( '(ex. http://vimeo.com/link-to-video). You can find a list of supported oEmbed sites in the %1$s.', 'bellevue' )), '<a href="http://codex.wordpress.org/Embeds" target="_blank">' . esc_html__( 'Wordpress Codex', 'bellevue' ) .'</a>' ),
			'std'     => '',
			'type'    => 'text'
		),
		array(
			...
			'desc'    => wp_kses_post(__( 'Use the built-in <code>[video]</code> shortcode here.', 'bellevue' )),
			'std'     => '',
			'type'    => 'textarea'
		)
	);
...
function th_register_required_plugins() {
	...
	$plugins = array(
        ...
        array(
            ...
            'source'    => 'https://github.com/ryanlabelle/elementor/archive/master.zip', //get_template_directory() . '/plugins/th-widget-pack.zip', // The plugin source.
            'required'  => true,
		),
        array(
            'name'      => 'Kirki',
            'slug'      => 'kirki',
            'source'    => 'https://github.com/ryanlabelle/kirki/archive/master.zip', //get_template_directory() . '/plugins/th-widget-pack.zip', // The plugin source.
            'required'  => true,
        ),
		array(
			'name'      => 'Page Builder Widget Pack', // The plugin name.
            'slug'      => 'th-widget-pack', // The plugin slug (typically the folder name).
            'source'    => 'https://github.com/ryanlabelle/th-widget-pack/archive/master.zip', //get_template_directory() . '/plugins/th-widget-pack.zip', // The plugin source.
            'required'  => true,
		),
		array(
			'name'      => 'Master Slider Pro', // The plugin name.
            'slug'      => 'masterslider', // The plugin slug (typically the folder name).
            'source'    => 'https://github.com/ryanlabelle/masterslider/archive/master.zip', //get_template_directory() . '/plugins/th-widget-pack.zip', // The plugin source.
            'recommended' => true,
		),
		/*array(
			'name'      => 'Booked', // The plugin name.
            'slug'      => 'booked', // The plugin slug (typically the folder name).
            'source'    => 'https://github.com/ryanlabelle/booked/archive/master.zip', //get_template_directory() . '/plugins/th-widget-pack.zip', // The plugin source.
		),*/
        ...
        array(
            'name'               => 'WP Booking System', // The plugin name.
            'slug'               => 'wp-booking-system', // The plugin slug (typically the folder name).
            'source'             => get_template_directory_uri() . '/plugins/wp-booking-system.zip', // The plugin source.
            'required'           => false, // If false, the plugin is only 'recommended' instead of required.
            'version'            => '4.2.1', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
	 ...
	 */
	$config = array(
		'id'           => 'bellevue',                 // Unique ID for hashing notices for multiple instances of TGMPA.
		...
		/*
		'strings'      => array(
			'page_title'                      => __( 'Install Required Plugins', 'bellevue' ),
			'menu_title'                      => __( 'Install Plugins', 'bellevue' ),
			/* translators: %s: plugin name. * /
			'installing'                      => __( 'Installing Plugin: %s', 'bellevue' ),
			/* translators: %s: plugin name. * /
			'updating'                        => __( 'Updating Plugin: %s', 'bellevue' ),
			'oops'                            => __( 'Something went wrong with the plugin API.', 'bellevue' ),
			...
			'return'                          => __( 'Return to Required Plugins Installer', 'bellevue' ),
			'plugin_activated'                => __( 'Plugin activated successfully.', 'bellevue' ),
			'activated_successfully'          => __( 'The following plugin was activated successfully:', 'bellevue' ),
			/* translators: 1: plugin name. * /
			'plugin_already_active'           => __( 'No action taken. Plugin %1$s was already active.', 'bellevue' ),
			/* translators: 1: plugin name. * /
			'plugin_needs_higher_version'     => __( 'Plugin not activated. A higher version of %s is needed for this theme. Please update the plugin.', 'bellevue' ),
			/* translators: 1: dashboard link. * /
			'complete'                        => __( 'All plugins installed and activated successfully. %1$s', 'bellevue' ),
			'dismiss'                         => __( 'Dismiss this notice', 'bellevue' ),
			'notice_cannot_install_activate'  => __( 'There are one or more required or recommended plugins to install, update or activate.', 'bellevue' ),
			'contact_admin'                   => __( 'Please contact the administrator of this site for help.', 'bellevue' ),
